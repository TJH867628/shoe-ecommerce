...
<body class="min-h-screen text-slate-900">
    <div class="min-h-screen lg:grid lg:grid-cols-[280px_1fr]">
        <div id="adminSidebarBackdrop" class="fixed inset-0 z-40 hidden bg-slate-950/50 lg:hidden"></div>

        <aside id="adminMobileSidebar" class="fixed inset-y-0 left-0 z-50 flex w-[280px] -translate-x-full flex-col bg-slate-950 text-white border-r border-slate-800 transition-transform duration-200 lg:hidden">
            <div class="p-6 border-b border-slate-800">
                <div class="flex items-center justify-between gap-3">
                    ...
                    <button type="button" id="closeAdminSidebar" class="w-10 h-10 rounded-2xl bg-white/10 text-white flex items-center justify-center hover:bg-white/20">
            ...
            <div class="mt-auto p-6 border-t border-slate-800">
                ...
                <p class="text-sm text-slate-400 break-all">{{ auth()->user()->email ?? '' }}</p>
            </div>
        </aside>
        ...
        <main class="min-w-0">
            <header class="sticky top-0 z-20 bg-white/90 backdrop-blur border-b border-slate-200">
                <div class="px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between gap-4">
                    <div class="flex items-center gap-3">
                        <button type="button" id="openAdminSidebar" class="lg:hidden w-10 h-10 rounded-2xl bg-slate-900 text-white flex items-center justify-center">
    ...
    <script>
        ...
        openAdminSidebar?.addEventListener('click', showAdminSidebar);
        closeAdminSidebar?.addEventListener('click', hideAdminSidebar);
        adminSidebarBackdrop?.addEventListener('click', hideAdminSidebar);
