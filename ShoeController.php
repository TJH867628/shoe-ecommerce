...
class ShoeController extends Controller
{
    ...
    public function getAdminShoeDetails(int $shoeId)
    {
        return Shoe::with([
            'brand',
            'options',
    ...
    public function getShoeOptions(int $shoeId)
    {
        return ShoeOption::where('shoe_id', $shoeId)
            ->orderBy('option_name')
            ->get();
    }
    ...
    public function createShoeOptions(Request $request)
    {
        ...
        $optionNames = collect($validated['option_names'])
            ->map(fn ($optionName) => trim($optionName))
            ->filter()
            ->unique(fn ($optionName) => strtolower($optionName))
            ->values();

        if ($optionNames->isEmpty()) {
            return back()->with('error', 'Please enter at least one option name.');
        }

        $options = [];

        foreach ($optionNames as $optionName) {
            if ($this->hasOption((int) $validated['shoe_id'], $optionName)) {
                return back()->with('error', 'This option already exists for this shoe.');
            }
    ...
    public function updateOption(Request $request, int $optionId)
    {
        ...
        $exists = ShoeOption::where('shoe_id', $option->shoe_id)
